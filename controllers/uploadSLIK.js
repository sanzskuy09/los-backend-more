...
router.post("/upload-slik/:application_id/:nik", upload.single("file"), (req, res) => {
  try {
    ...
    const baseDir = "/opt/be/uploads/HasilSLIK";
    const targetDir = path.join(baseDir, application_id, nik);

    // Buat folder jika belum ada
    fs.mkdirSync(targetDir, { recursive: true });

    const filePath = path.join(targetDir, "SLIK.pdf");
